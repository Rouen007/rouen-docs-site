# Heatmap 五层框架重学（2026-07-18 第三轮系统性学习）

> 前两轮：`heatmap_matrix_tutorial_2026-07-11.md`（0-10课单点深挖）+ 五阶段流程版（对话内）。
> 本轮把体系重排成**五层纵深**，吸收了 7/18 的三个新认知：①1DTE 时间维度问题 ②个股无每日到期 ③平台 0DTE 尚未真正上线。
> 案例规则：一律带完整前后文（盘前结构快照 → 怎么读 → 次日实际走势），见 memory `feedback_self_contained_examples`。

---

## 框架总览

![五层框架总览](assets/heatmap_relearn_2026-07-18/01_five_layer_framework.svg)

```
第一层 · 原理引擎   对冲是数学强制，不是心理
第二层 · 时间结构   到期组决定力量权重（含 7/18 两次修正）
第三层 · 六步读图   现价 → King → 边界
第四层 · 双重验证   横向共振 · 纵向追踪
第五层 · 分类与执行 结构定剧本，原则定动作（待续）
─────────────────────────────────────
边界（横贯所有层）：不预测方向 · 只是估算 · 挡不住事件
```

与旧五阶段版的三处重排：
1. **时间结构升为独立层**——"1DTE 是明天的数据为什么影响今天"暴露出到期维度是独立的理解轴，不是读图的背景知识。
2. **原理层从第0课升为地基**——后面每层的"为什么"都回溯到"dealer 对冲方向由 Gamma 符号唯一决定"这一句，遇到拿不准的判断挖到这层自己推。
3. **边界从最后一课变成横贯线**——十大误读对每一层的输出都有否决权（SPCX 案例：结构全对，照样被新闻碾过）。
...
> **推论（1DTE 问题的答案）**：只要合约今天有 Gamma，dealer 今天就在对冲，"哪天到期"决定的是结算日，不是力量生效日。

**3. 符号来源**：客户净买期权 → dealer 净卖 → 负 Gamma；客户净卖（covered call/卖put）→ dealer 净买 → 正 Gamma。每格数字 = 该 strike×到期上 dealer 净 Gamma 的美元估算。

![正负Gamma:弹簧 vs 雪球](assets/heatmap_relearn_2026-07-18/02_gamma_spring_snowball.svg)
...
![BMNR 07-10 日内走势](assets/heatmap_relearn_2026-07-18/07_bmnr_0710_chart.svg)
...
![SPCX 07-10 日内走势](assets/heatmap_relearn_2026-07-18/08_spcx_0710_chart.svg)

### 案例三 RKLB（避坑，假地板）→ 详见第四层

---

## 第二层 · 时间结构（含 7/18 两次修正）

**底层规律（永远成立）**：Gamma 约与 √剩余时间 成反比——越近到期越尖锐。近端列是尖峰，远端列是丘陵。

![Gamma幅度 vs 剩余到期时间](assets/heatmap_relearn_2026-07-18/03_gamma_vs_time.svg)

**官方三档设计**：0DTE（当日）/ 1DTE（下一交易日）/ Weekly（7天内其余）。0DTE/1DTE 共享量能指标（Active Call/Put、Max Change），**Weekly 只有 OI 存量结构，无当日资金流**。

**⚠️ 修正一（个股维度）**：BMNR/RKLB/SPCX 等绝大多数个股只有周度到期，没有每日到期合约。个股图上只有"近端列 vs 远端列"两档，别硬套 0DTE/1DTE 的每日换挡逻辑。

**⚠️ 修正二（平台现状，用户 7/18 澄清）**：**包括 SPX 在内，当前所有到期分组显示的都是"下一个有期权到期日"的 GEX**，不存在独立计算的当日 0DTE 切片；0DTE 精细拆分是官方后续迭代项。见 memory `project_nightwatch_0dte_not_live`。
...
![六步读图法 RKLB 梯子图](assets/heatmap_relearn_2026-07-18/04_six_step_ladder_rklb.svg)
...
**术语提醒**："近端列" = 该标的下一个有期权到期日那列，不叫 0DTE（见第二层修正）。
...
![双重验证示意](assets/heatmap_relearn_2026-07-18/05_dual_validation.svg)
...
![RKLB 07-10 日内走势:地板被刺破后拉回](assets/heatmap_relearn_2026-07-18/09_rklb_0710_chart.svg)

### 纵向追踪（跨时间）——防"快照上活人死人长一样"
同一格隔几十分钟/隔天再看：**长大 = 有人补弹药；萎缩 = 正在被消耗**。配合触碰次数读："守住两次但数值 620K→410K→180K（示意数字）"不是支撑强，是支撑正在死，第三次测试大概率是终点。与 PULSE 手册"Spent 状态、第 3 次后大概率破"同源。
...
![六个实证结构原型](assets/heatmap_relearn_2026-07-18/06_six_archetypes.svg)
...
## 本轮沉淀的三条新记忆
- `feedback_self_contained_examples`：案例三段式（结构快照→怎么读→实际走势），不许 ticker 速记
- `project_nightwatch_0dte_not_live`：0DTE 尚未真正上线，所有到期列显示的都是下一个有期权到期日的 GEX
- 个股无每日到期：0DTE/1DTE 框架只对指数/超大盘有意义（已写入 tutorial/cheatsheet/portable 三份文档的修正注记）
